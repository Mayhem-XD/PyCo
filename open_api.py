...
@user_bp_o.route('/hotplace',methods=['GET','POST'])
def hotplace():
    menu = {'ho':0,'nb':0,'us':0,'cr':0,'sc':0,'oa':1}
    if request.method == 'GET':
        return render_template('prototype/api/title.html',menu=menu,weather=get_weather(app),quote=session['quote'],addr=session['addr'])
    else:
